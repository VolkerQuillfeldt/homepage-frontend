...
class Country extends Component {
	...
	render() {
		...
		return (
			...
			<a className="text-white border-0" data-bs-toggle="collapse" href={"#"+divname} role="button"  aria-expanded="false" aria-controls={divname}>
				...
				{this.state.myTravels.map((item,index) => (
					<p key={index}><button type="button"
																 className="btn btn-warning btn-sm" href='#'
																 onClick={() => this.props.changeBody(item.path,1,6)}>{item.travel}</button></p>))
				}
				</div>
				</span>
		)
